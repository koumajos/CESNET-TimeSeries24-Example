...
DIRECTORY="cesnet-time-series-2023-2024/institutions/agg_1_hour/"
...
do
    ...
    do
    if [ -f "$FILE" ]; then
        echo "-v metric='$metric',id_ip='$(basename $FILE)' sarima_retraining_institutions.sh"
        qsub -v "metric='$metric',id_ip='$(basename "$FILE")'" sarima_retraining_institutions.sh
    fi
    done
done
