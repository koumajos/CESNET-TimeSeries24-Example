...
DIRECTORY="cesnet-time-series-2023-2024/ip_addresses_sample/agg_1_hour/"
...
do
    ...
    do
    if [ -f "$FILE" ]; then
        echo "-v metric='$metric',id_ip='$(basename $FILE)' sarima_retraining_ip_addresses_sample.sh"
        qsub -v "metric='$metric',id_ip='$(basename "$FILE")'" sarima_retraining_ip_addresses_sample.sh
    fi
    done
done
